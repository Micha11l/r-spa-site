...
export default function StaffGiftCardsPage() {
  ...
  const loadGiftCards = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/staff/gift-cards?${params}`);
  ...
  const loadStats = async () => {
    try {
      const response = await fetch("/api/staff/gift-cards/stats");
  ...
  const handleExport = async () => {
    try {
      const response = await fetch("/api/staff/gift-cards/export");
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-3xl font-bold text-slate-900">Gift Cards Management</h1>
              <p className="text-slate-600 mt-1">View and manage all gift cards</p>
            </div>
            <button
                ...
                <tbody className="divide-y divide-slate-200">
                  {giftCards.map((card) => (
                    <tr
                      ...
                      onClick={() => router.push(`/staff/gift-cards/${card.id}`)}
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            router.push(`/staff/gift-cards/${card.id}`);
                          }}
